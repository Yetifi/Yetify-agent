...
export default function WalletConnection() {
  ...
  const getChainInfo = (walletType: string) => {
    switch (walletType) {
      case 'metamask':
        return { chain: 'Ethereum', token: 'ETH', color: 'bg-blue-50 border-blue-200 text-blue-800' };
      case 'near':
        return { chain: 'NEAR', token: 'NEAR', color: 'bg-green-50 border-green-200 text-green-800' };
      case 'walletconnect':
        return { chain: 'Multi-Chain', token: 'Various', color: 'bg-purple-50 border-purple-200 text-purple-800' };
      default:
        return { chain: 'Unknown', token: 'Unknown', color: 'bg-gray-50 border-gray-200 text-gray-800' };
    }
  };

  if (walletState.isConnected) {
    const chainInfo = getChainInfo(walletState.walletType!);
    
    return (
      <div className={`flex items-center space-x-3 border rounded-lg px-4 py-2 ${chainInfo.color}`}>
